...
export function Button({ title, onPress, variant = 'primary', size = 'md', style, textStyle, disabled, color }: Props) {
  const btnStyle = [
    ...
    variant === 'outline' && (color ? { borderColor: color + '40', backgroundColor: color + '0c' } : styles.outline),
  ...
  return (
    <TouchableOpacity style={btnStyle} onPress={onPress} disabled={disabled} activeOpacity={0.65}>
